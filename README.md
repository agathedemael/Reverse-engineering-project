...
### Warning:
If you want to run this notebook locally, check the compatibility of the imported packages with your local python environment. 
...
### Warning: 
If the stl files of the three different geometries are not downloaded in the same folder, it is necessary to first run blade_guard_geometry.m, then blade_guard_CFD.m. Both the CFD and the optimisation algorithms are heavy to run on MATLAB, expect around 5 minutes and 15 minutes of run time respectively.
